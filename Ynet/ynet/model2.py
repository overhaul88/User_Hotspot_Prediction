...
class YNetTorch(nn.Module):
	def __init__(self, obs_len, pred_len, segmentation_model_fp, use_features_only=False, semantic_classes=6,
		...
		if segmentation_model_fp is not None:
			self.semantic_segmentation = torch.load(r'D:\AKG\GSN\Human-Path-Prediction\ynet\segmentation_models\SDD_segmentation.pth')
			# self.semantic_segmentation = torch.load(r'D:\AKG\GSN\Human-Path-Prediction\ynet\pretrained_models\sdd_longterm_weights.pt')
...
class YNet(nn.Module):
    def __init__(self, input_size, hidden_size, num_waypoints, num_paths, num_modes):
        super(YNet, self).__init__()

        # Goal Encoder
        self.goal_encoder = nn.Linear(input_size, hidden_size)

        # Waypoint Encoder
        self.waypoint_encoder = nn.GRU(input_size, hidden_size, num_layers=1, batch_first=True)

        # Path Conformity Module
        self.path_conformity = nn.Linear(hidden_size, num_paths)

        # Multi-Modal Prediction
        self.mode_prediction = nn.Linear(hidden_size, num_modes)
        self.trajectory_decoder = nn.GRU(hidden_size, hidden_size, num_layers=1, batch_first=True)
        self.trajectory_output = nn.Linear(hidden_size, input_size)

    def forward(self, input, waypoints):
        # Goal Encoding
        goal_encoding = self.goal_encoder(input)

        # Waypoint Encoding
        _, waypoint_encoding = self.waypoint_encoder(waypoints)

        # Path Conformity
        path_probs = F.softmax(self.path_conformity(waypoint_encoding), dim=1)

        # Multi-Modal Prediction
        mode_probs = F.softmax(self.mode_prediction(goal_encoding), dim=1)

        # Trajectory Decoding
        traj_output, _ = self.trajectory_decoder(goal_encoding.unsqueeze(1))
        traj_output = self.trajectory_output(traj_output.squeeze(1))

        return traj_output, mode_probs, path_probs
